...
def cnn_subscriber(cnn_url, source):
    ...
    current_update = d.feed.updated
    print "Timestamp: " + str(current_update)
...
def guardian_subscriber(guardian_url, source):
    ...
    for entry in d.entries:
        ...
        title = entry.title
        handler.insert_country(source,timestamp,link, source, title)
...
handler = DB_Handler()
handler.delete_from_update()
handler.delete_from_news()
